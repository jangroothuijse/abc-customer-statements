...
public class StatementProcessor {
    ...
    public ValidationReport validate(Statement customerStatement) {
        final var lines = customerStatement.messages()
                                           .stream()
                                           .collect(Collectors.groupingBy(Transaction::reference))
                                           .entrySet().stream()
                                           .flatMap(entry -> validateMessages(entry.getKey(), entry.getValue()))
                                           .toList();
    ...
    validateMessage(Transaction message, List<String> uniqueness) {
        final var constraintViolations = validator.validate(message)
                                                  .stream()
                                                  .map(ConstraintViolation::getMessage);
        final var errors = Stream.concat(constraintViolations, uniqueness.stream())
                                 .toList();
